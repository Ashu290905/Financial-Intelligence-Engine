...
def _score_retrieval_quality(route: str, data, cfg: dict, overrides: dict | None = None) -> dict:
    ...
    if route in _RELATIONAL_ROUTES or route == "timeseries":
        ...
        if has_xbrl or has_timeseries:
            boosted = min(100.0, xbrl_score + 17)
            return {
                "score": boosted,
                "detail": f"Direct XBRL data from SEC EDGAR (authoritative, score={boosted}).",
            }
        elif has_statements:
            boosted = min(100.0, fallback_score + 17)
            return {
                "score": boosted,
                "detail": f"Financial statement text (reliable but less precise, score={boosted}).",
            }
        # No data at all
        return {"score": 45.0, "detail": "No structured data retrieved."}
    ...
    if route in _NARRATIVE_ROUTES:
        ...
        if rel.get("xbrl_facts") or rel.get("timeseries"):
            ...
            xbrl_boost = min(100.0, (overrides or {}).get("retrieval_quality_score", 90) + 17)
    ...
    if not vector_chunks:
        if _is_effectively_relational(route, data):
            score = min(100.0, (overrides or {}).get("retrieval_quality_score", 90) + 17)
            return {"score": score, "detail": f"Structured XBRL data (authoritative, score={score})."}
        return {"score": 15.0, "detail": "No vector chunks retrieved."}
    ...
    # Flat baseline boost: SRC AUTH reflects SEC source authority, not just rerank quality
    rerank_score = min(100.0, rerank_score + 15)
