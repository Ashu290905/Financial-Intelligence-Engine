...
TARGET_SECTIONS_10K = ["item_1a_risk_factors", "item_7_mda", "item_1_business", "item_1c_cybersecurity"]
...
def generate_embeddings(texts: list[str], batch_size: int = EMBEDDING_BATCH_SIZE, return_usage: bool = False):
    ...
    Batches by token count (max 250k tokens per request) rather than chunk count,
    since OpenAI's limit is 300k tokens per request.
    Auto-splits batches in half on token limit errors.
    ...
    def _embed_batch(batch_texts, batch_indices):
        nonlocal total_prompt_tokens
        # Filter out empty/None — OpenAI rejects batches containing them
        valid = [(t, i) for t, i in zip(batch_texts, batch_indices)
                 if t and isinstance(t, str) and t.strip()]
        if not valid:
            return
        valid_texts, valid_indices = zip(*valid)
        valid_texts, valid_indices = list(valid_texts), list(valid_indices)
        try:
            ...
            for item in response.data:
                all_embeddings[valid_indices[item.index]] = item.embedding
            if return_usage and response.usage:
                total_prompt_tokens += response.usage.prompt_tokens
        except Exception as e:
            err = str(e)
            if ("max_tokens_per_request" in err or "$.input" in err) and len(valid_texts) > 1:
                mid = len(valid_texts) // 2
                print(f"  Splitting batch in half to isolate issue...")
                _embed_batch(valid_texts[:mid], valid_indices[:mid])
                _embed_batch(valid_texts[mid:], valid_indices[mid:])
            else:
                print(f"  Error embedding batch of {len(valid_texts)}: {e}")

    current_texts, current_indices, current_tokens = [], [], 0
    for i, text in enumerate(texts):
        est = _estimate_tokens(text)
        if current_texts and current_tokens + est > MAX_TOKENS_PER_BATCH:
            print(f"  Embedding batch of {len(current_texts)} texts (~{current_tokens:,} tokens)...")
            _embed_batch(current_texts, current_indices)
            current_texts, current_indices, current_tokens = [], [], 0
        ...
        with conn.cursor() as cur:
            for (ticker, fiscal_year, section_id), records in groups.items():
                ...
                for meta, chunk, embedding in records:
                    ...
                    cur.execute("""
                        ...
                        DO UPDATE SET
                            ...
                            subsection_heading = EXCLUDED.subsection_heading
                    """, (
                        ticker, company_name, accession_number, filing_date,
                        fiscal_year, section_id, section_title, part_number,
                ...
                cur.execute("""
                    UPDATE sections_10k
                    SET is_chunked = TRUE
                    WHERE ticker = %s AND fiscal_year = %s AND section_id = %s AND is_chunked = FALSE
                """, (ticker, fiscal_year, section_id))
